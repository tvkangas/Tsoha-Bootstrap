...
class tulos extends BaseModel {
    ...
    public static function haeTuloksenVaylatulokset($tulosid) {
        $query = DB::connection()->prepare('SELECT Vayla.numero AS numero, vt.heitot AS heitot FROM VaylaTulos vt 
        LEFT JOIN Tulos ON vt.tulosId = Tulos.Id 
        LEFT JOIN Vayla ON Vayla.id = vt.vaylaId
        WHERE vt.tulosId = :tulosid');
        $query->execute(array('tulosid' => $tulosid));
        $rows = $query->fetchAll();
        $tulokset = array();
        foreach ($rows as $row) {
            $str = 'vayla' . $row['numero'];
            $tulokset[$str] = $row['heitot'];
        }
        return $tulokset;
